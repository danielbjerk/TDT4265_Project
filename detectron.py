...
#print(DatasetCatalog.get("tdt4265_train"))

i = 0
for d in random.sample(DatasetCatalog.get("tdt4265_train"), 3):
    img = cv2.imread(d["file_name"])
    visualizer = Visualizer(img[:, :, ::-1], scale=0.5)
    out = visualizer.draw_dataset_dict(d)
    cv2.imwrite("ligma_ass" + str(i) + ".png", out.get_image()[:, :, ::-1])
    i += 1
...
if __name__ == '__main__':
    ...
    cfg = get_cfg()     # configen vi skal bruke videre her
